...
public class WRLDNavWidgetBase: UIView, WRLDNavModelObserverProtocol
{
    ...
    internal let navModelShowRoutingPopUpKeyPath = #keyPath(WRLDNavModel.showRoutingPopUp)
    
    var calculateRoutingPopUpView:WRLDNavPopUpView!
    ...
    {        
        ...
        if(keyPath == navModelShowRoutingPopUpKeyPath)
        {
            if let navModel = observer.navModel()
            {
                if navModel.showRoutingPopUp {
                    calculateRoutingPopUpView.show()
                }else {
                    calculateRoutingPopUpView.hide()
                }
            }
    ...
    @objc public func disableNavigationExitOption(){}
}
