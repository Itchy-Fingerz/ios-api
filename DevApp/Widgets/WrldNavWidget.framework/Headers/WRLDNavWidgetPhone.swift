...
public class WRLDNavWidgetPhone: WRLDNavWidgetBase
{
    ...
    private func initCommon()
    {
        ...
        if #available(iOS 11.0, *) {
            ...
            if let safeABottom = UIApplication.shared.keyWindow?.safeAreaInsets.bottom {
                _safeAreaBottom = Int(safeABottom)
            }
        } else {
        ...
        calculateRoutingPopUpView = WRLDNavPopUpView(frame:self.bounds)
        observer.registerObserver(observerKey: navModelShowRoutingPopUpKeyPath)
        view.addSubview(calculateRoutingPopUpView)
    }
    ...
    @objc public override func disableNavigationExitOption()
    {
        setupJourneyView.hideCloseButton()
    }
}
