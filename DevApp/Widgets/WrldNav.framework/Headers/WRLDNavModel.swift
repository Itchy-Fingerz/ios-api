...
public class WRLDNavModel: NSObject
{
    ...
    /**
     This property will be used to show/hide dailoug pop.
     */
    private var m_showRoutingPopUp: Bool = false
    @objc(showRoutingPopUp) public var showRoutingPopUp: Bool {
        set
        {
            if m_showRoutingPopUp != newValue
            {
                willChangeValue(forKey: "showRoutingPopUp")
                m_showRoutingPopUp = newValue
                didChangeValue(forKey: "showRoutingPopUp")
            }
        }
        get { return m_showRoutingPopUp }
    }
